...
**并发实例上限**：同一用户/同一台机器下，所有 serveRoot 的后台进程合计**最多 5 个**（硬编码常量，不可配置）。

这是**对上层调用方的保险丝**：`preview` 幂等复用同一 serveRoot，但**不同** serveRoot 每次都会起一个新后台进程；若上层框架反复起新实例而忘记 `stop`，进程会无限堆积。上限兜底：**启动新实例的同时**主动停掉最老的一个（按 `startedAt` 升序）；若它停不掉（比如 HTTP 无响应且进程已被系统复用），**静默忽略、照常启动，绝不影响新实例**。足够覆盖常见多项目开发（5 个同时预览），又保证堆积被压住。
